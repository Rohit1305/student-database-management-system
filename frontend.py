...
class Student:
    ...
    def __init__(self, root):
        ...
        self.root.config(bg="dark turquoise")
        ...
        def iExit():
            iExit = tkinter.messagebox.askyesno("Student DBMS", "Are you sure you want to exit?")
            if iExit > 0:
                root.destroy()
                return
        ...
        def addData():
            if (len(stdId.get()) != 0):
                pb.addStdRec(stdId.get(), Firstname.get(), Surname.get(), DoB.get(), Age.get(), Gender.get(),
                             Adress.get(), Mobile.get())
                studentlist.delete(0, END)
                studentlist.insert(END, (
                stdId.get(), Firstname.get(), Surname.get(), DoB.get(), Age.get(), Gender.get(), Adress.get(),
                Mobile.get()))
        ...
        TitFrame = Frame(MainFrame, bd=1, padx=54, pady=8, bg="dark turquoise", relief=RIDGE)
        TitFrame.pack(side=TOP)  # THIS IS STITLE FRAME

        self.lblTit = Label(TitFrame, font=('arial', 47, 'bold'), text="Students Database Management System", bg="dark turquoise",
                            fg="black")
        self.lblTit.grid()


        ButtonFrame = Frame(MainFrame, bd=1, width=1350, height=70, padx=18, pady=10, bg="dark turquoise", relief=RIDGE)
        ...
        self.lblStdId = Label(DataFrameLeft, font=('arial', 12, 'bold'), padx=2, pady=3, text="student Id:",
                              bg="ghost white")
        ...
        studentlist = Listbox(DataFrameRight, width=68, height=12, font=('arial', 12, 'bold'),
